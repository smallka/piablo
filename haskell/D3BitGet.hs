...
readBool :: S -> Bool
readBool (S bs n) = testBit (unsafeHead bs) (n+1)
...
readWord8 n (S bs o)
  ...
  | n <= 8 = let msb = (fromIntegral (unsafeHead bs) `shiftr_w8` o)
                 n' = n - 8 + o
                 lsb = fromIntegral (unsafeIndex bs 1)
                 w = (msb `shiftl_w8` n') .|. (lsb .&. make_mask n')
             in w
...
readWord16be n s@(S bs o)
  ...
  | o == 0 = let msb = fromIntegral (unsafeHead bs)
                 lsb = fromIntegral (unsafeIndex bs 1)
                 n' = n - 8
                 w = (msb `shiftl_w16` n') .|. (lsb .&. make_mask n')
             in w
...
readWord64be n s
  -- 32 or fewer bits, use readWord32be
  | n <= 32 = fromIntegral (readWord32be n s)

  | n <= 64 = let high = fromIntegral (readWord32be 32 s)
                  n' = n - 32
                  low = fromIntegral (readWord32be n' (incS 32 s))
                  w = (high `shiftl_w64` n') .|. low
              in w
...
readWithoutOffset (S bs o) shifterL shifterR n
  ...
  | n <= 64 = let segs = byte_offset n
                  ...
                  last = (fromIntegral (unsafeIndex bs segs)) .&. make_mask o'
...
readWithOffset (S bs o) shifterL shifterR n
  | n <= 64 = let n' = n - 8 + o
                  top = (fromIntegral (unsafeHead bs) `shifterR` o) `shifterL` n'
                  segs = byte_offset n'
                  ...
                  last | o' > 0 = (fromIntegral (unsafeIndex bs (segs + 1))) .&. make_mask o'
                       | otherwise = 0
